...
| قیمت تومانی | رمضینکس → tgju → آخرین نرخ ذخیره‌شده → ورود دستی (با بازه sanity) |
...
| ماشین‌حساب | تبدیل دوطرفه Pi ↔ تومان/دلار + ورودی با ارقام فارسی (۰-۹ و ٫) |
| پرتفوی | ارزش + سود/زیان + اسنپ‌شات روزانه («▲ ۲٪ از دیروز») + اشتراک وضعیت |
| هشدار قیمت | بالای/زیر + دکمه‌های سریع ±۵٪ + هشدار قیمت ردشده = ثبت فعال‌شده + تاریخچه «آنچه از دست دادی» |
| نوتیفیکیشن | از طریق SW registration (در PWA نصب‌شده هم کار می‌کند) |
| استریک روزانه | 🔥 شمارش بازدیدهای روزانه (تاریخ محلی) |
| ورود با Pi | `Pi.authenticate` با اسکوپ username |
| تبلیغات | interstitial هنگام جابه‌جایی تب‌ها (throttle ۳ دقیقه) + rewarded حمایتی |
| PWA | نصب‌شدنی + آفلاین کامل (SW v4) + فونت محلی + آیکون‌های واقعی |
