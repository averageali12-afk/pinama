...
**نسخه زنده:** [https://averageali12-afk.github.io/pinama/](https://averageali12-afk.github.io/pinama/) 🚀 (v1.3.0)
...
| بخش | توضیح |
|---|---|
| قیمت لحظه‌ای | Gate.io (اولویت اول — در ایران در دسترس) → CoinGecko → OKX + کش آفلاین با برچسب صادقانه |
| قیمت تومانی | رمضینکس → tgju → آخرین نرخ ذخیره‌شده → ورود دستی (با بازه sanity + راهنمای لمسی) |
| نمودار | Canvas سبک‌وزن — ۲۴ساعت/۷روز/۳۰روز + خط مرجع قیمت لحظه‌ای + تولتیپ لمسی + سری کهنه در آفلاین |
| چیپ‌های بازه | درصد تغییر ۲۴ساعت/۷روز/۳۰روز — کلیک = تعویض نمودار |
| موقعیت ۳۰ روزه | «چند ٪ مانده تا سقف/کف ۳۰ روزه» — قلاب بازدید روزانه |
| ماشین‌حساب | تبدیل دوطرفه + چیپ‌های مبلغ سریع + **سود هدف** (+۱۰/۲۵/۵۰/۱۰۰٪ بر مبنای میانگین خرید شما) + ورودی ارقام فارسی |
| پرتفوی | ارزش + سود/زیان + اسنپ‌شات روزانه و هفتگی + اشتراک وضعیت با لینک + خروجی CSV |
| هشدار قیمت | بالای/زیر + دکمه‌های سریع ±۵٪ + قیمت ردشده = ثبت فعال‌شده + dedupe + rearm هوشمند + تاریخچه «آنچه از دست دادی» + ویبره |
| نوتیفیکیشن | از طریق SW registration (در PWA نصب‌شده هم کار می‌کند) + tag یکتا |
| استریک روزانه | 🔥 شمارش بازدیدهای روزانه (تاریخ محلی، با رزومه از بک‌گراند) |
| آن‌بوردینگ | کارت خوش‌آمد ۴مرحله‌ای فقط بار اول |
| ورود با Pi | `Pi.authenticate` با اسکوپ username |
| تبلیغات | interstitial فقط ناوبری جلو (throttle ۳ دقیقه) + rewarded حمایتی |
| PWA | نصب‌شدنی + میان‌بُرها با deep-link + اسکرین‌شات در پنجره نصب + آفلاین کامل (SW v14) + فونت preload |
| ناوبری native | دکمه back اندروید = تب قبلی، نه خروج از اپ |
| برای داوران | یادداشت انگلیسی در تنظیمات + کارت OG/Twitter برای اشتراک لینک + خروجی داده (مالکیت داده) |
